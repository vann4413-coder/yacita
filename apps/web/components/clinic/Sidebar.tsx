    ...
    <aside className="w-60 shrink-0 bg-bgdark flex flex-col h-full">
      ...
      <div className="px-6 py-6 border-b border-white/10">
        <a href="/">
          <span className="text-2xl font-extrabold font-heading">
            <span className="text-white">ya</span><span className="text-cta">cita</span>
          </span>
          <p className="text-xs text-[#79C8C5] font-body mt-0.5">Panel de clínica</p>
        </a>
      </div>
          ...
          <Link
            ...
            className={clsx(
              ...
              pathname.startsWith(href)
                ? 'bg-turquoise/20 text-[#79C8C5] border border-turquoise/30'
                : 'text-[#79C8C5]/70 hover:bg-white/10 hover:text-white',
            )}
          >
      ...
      <div className="px-4 py-4 border-t border-white/10 flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-[#79C8C5]/70 truncate font-body">{user?.email}</p>
        </div>
        <button
          onClick={handleLogout}
          className="text-[#79C8C5]/70 hover:text-white transition-colors text-lg"
          title="Cerrar sesión"
        >
          ↩
        </button>
